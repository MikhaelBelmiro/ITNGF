...
class LitModelModule(LightningModule):
    def __init__(self, data_related_config, model_hyperparam_config) -> None:
        ...
        self.loss = mse_loss(self.config["scaler"])
        self.rmse_loss = rmse_loss(self.config["scaler"])
    ...
    def forward(self, batch):
        ...
        pred_ts = self.config["scaler"].inverse_transform(pred_ts)
        return pred_ts
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        metrics_dict = {
            "t_loss": loss.item(),
            "t_rmse": self.rmse_loss(pred_ts, gt_ts),
    ...
    def validation_step(self, batch, batch_idx) -> Union[STEP_OUTPUT, None]:
        ...
        metrics_dict = {
            "v_loss": loss.item(),
            "v_rmse": self.rmse_loss(pred_ts, gt_ts),
    ...
    def test_step(self, batch, batch_idx) -> Union[STEP_OUTPUT, None]:
        ...
        metrics_dict = {
            "test_loss": loss.item(),
            "test_rmse": self.rmse_loss(pred_ts, gt_ts),
